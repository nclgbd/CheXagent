...
class CheXagent(object):
    ...
    def generate(self, paths, prompt, **kwargs):
        ...
        output = self.get_generation_output(input_ids, **kwargs)
        response = self.tokenizer.decode(output[0][input_ids.size(1) : -1])
        return response

    def view_classification(self, path):
        assert isinstance(path, str)
        prompt = (
            "What is the view of this chest X-ray? Options: (a) PA, (b) AP, (c) LATERAL"
        )
    ...
    def plot_image(self, path, response, save_path):
        ...
        boxes = [
            _dict["box"]
            for _dict in self.tokenizer.to_list_format(response)
            if "box" in _dict
        ]
        boxes = [
            [
                int(cord) / 100
                for cord in box.replace("(", "").replace(")", "").split(",")
            ]
            for box in boxes
        ]
        w = pil_image.width
    ...
    def phrase_grounding(
        self, path, phrase, save_path=f"results_visualization/phrase_grounding.png"
    ):
        assert isinstance(path, str)
    ...
    def abnormality_detection(
        ...
        path,
        disease_name,
        save_path="results_visualization/abnormality_detection.png",
    ):
        assert isinstance(path, str)
        assert isinstance(disease_name, str)
    ...
    def chest_tube_detection(
        self, path, save_path="results_visualization/chest_tube_detection.png"
    ):
        assert isinstance(path, str)
    ...
    def rib_fracture_detection(
        self, path, save_path="results_visualization/rib_fracture_detection.png"
    ):
        assert isinstance(path, str)
    ...
    def foreign_objects_detection(
        self, path, save_path="results_visualization/foreign_objects_detection.png"
    ):
        assert isinstance(path, str)
        prompt = (
    ...
    def temporal_image_classification(self, paths, disease_name):
        ...
        assert isinstance(disease_name, str)
        prompt = (
            f"You are given two images: one reference image and one new image. "
    ...
    def named_entity_recognition(self, text):
        assert isinstance(text, str)
        prompt = (
            f"Given the list of entity types "
